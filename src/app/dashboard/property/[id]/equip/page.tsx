...
const Equipments = () => {
  ...
  if (!equipments || equipments.length === 0) {
    return <div className="p-6 text-center text-gray-500">No equipment data available.</div>;
  }

  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold text-center mb-8">Equipments</h1>
      ...
      {equipments.map((equipment) => {
        ...
        return (
          <div key={id} className="bg-white shadow rounded-lg p-6 mb-6">
            ...
            <p className="text-lg">
              <span className="font-medium">Maintenance Frequency:</span> {maintenanceFrequency}
            </p>
            <p className="text-lg">
            ...
            <p className="text-lg">
              ...
              {nextMaintenanceDate ? new Date(nextMaintenanceDate).toLocaleDateString() : "Not scheduled"}
            </p>
            <p className="text-lg">
              <span className="font-medium">Warranty Policy:</span> {warrantyPolicy || "Not specified"}
            </p>
            <p className="text-lg">
              <span className="font-medium">Maintenance Cost:</span>{" "}
              {maintenanceCost ? `$${maintenanceCost.toFixed(2)}` : "Not specified"}
            </p>
            {mapCoordinates && (
              <p className="text-lg">
